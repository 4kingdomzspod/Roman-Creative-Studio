...
### Layout A — KPI Strip
Four equal-width cards in a horizontal strip. Used: Overview row 1, any section header.
```typescript
<KPIStrip>
  <KPICard label="MRR" value="$0" trend="neutral" />
  <KPICard label="Projects" value="0" trend="neutral" />
  <KPICard label="Leads" value="0" trend="neutral" />
  <KPICard label="Pipeline" value="$0" trend="neutral" />
</KPIStrip>
```

### Layout B — Chart + Sidebar
Wide chart (8 col) + narrow sidebar content (4 col). Used: Revenue overview, SEO.

### Layout C — Two-Column Equal
Two equal-width panels (6 col each). Used: Pipeline + Projects on Overview.

### Layout D — Three-Column Equal
Three equal-width panels (4 col each). Used: Marketing / Tasks / Announcements on Overview.

### Layout E — Full-Width Table
Full width data table with filters. Used: Invoice aging, keyword rankings.

### Layout F — Card Grid
Responsive card grid (3 col desktop, 2 col tablet, 1 col mobile). Used: Project health grid.
