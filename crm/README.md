...
3. In the Apps Script editor, delete the default `Code.gs` placeholder content, then create fifteen script files matching the names in this folder — **Code**, **CRM_Builder**, **CRM_Settings**, **CRM_Dashboard**, **CRM_Import**, **CRM_Actions**, **CRM_Sync**, **CRM_Audits**, **CRM_Outreach**, **CRM_OutreachWorkflow**, **CRM_Scoring**, **CRM_CommandCenter**, **CRM_Analytics**, **CRM_Health**, **CRM_Automation** — and paste the matching file's contents into each (use the **+** next to "Files" in the left sidebar to add each one; Apps Script appends `.gs` automatically).
