 ...
 * Canonical home: /crm
 * Modular CRM architecture preserved. Feature logic remains in CRM_*.gs.
 *
 * SAFETY:
 * - CRM data only: Google Sheets / Apps Script.
 * - Website audit/research is read-only; CRM must never edit/deploy/publish/
 *   delete/mutate website source or hosting.
 * - Roman Creative Studio is a permanent internal test account and must be
 *   excluded from production metrics, revenue goals, sales queues, and lead
...
function onOpen() {
  ...
  ui.createMenu('RCS CRM')
    ...
    .addSubMenu(ui.createMenu('Auto Sync').addItem('Enable Auto Sync', 'enableAutoSync_').addItem('Disable Auto Sync', 'disableAutoSync_'))
    .addSubMenu(ui.createMenu('Website Audit').addItem('Audit Selected Prospect', 'menuAuditSelectedProspect_').addItem('Audit Website URL', 'menuAuditWebsiteUrl_').addItem('Audit Eligible Prospects', 'menuAuditEligibleProspects_').addItem('Record Manual Audit', 'menuRecordManualAudit_'))
    .addSubMenu(ui.createMenu('Outreach Tools').addItem('Generate Outreach Brief', 'menuGenerateOutreachBrief_').addItem('Generate Brief for Selected Prospect', 'menuGenerateOutreachBrief_').addItem('Mark as Contacted', 'menuMarkAsContacted_').addItem('Schedule Follow-Up', 'menuScheduleFollowUp_').addItem('Generate Follow-Up Message', 'menuGenerateFollowUpMessage_'))
    .addSubMenu(ui.createMenu('Lead Intelligence').addItem('Score Selected Prospect(s)', 'menuScoreSelectedProspects_').addItem('Score All Prospects', 'menuScoreAllProspects_').addItem('Show Top Leads', 'menuShowTopLeads_'))
    ...
    .addSubMenu(ui.createMenu('Automation').addItem('Run CRM Maintenance', 'menuRunCrmMaintenance_').addItem('Automation Status', 'menuAutomationStatus_').addItem('Enable Daily Maintenance', 'enableDailyMaintenance_').addItem('Disable Daily Maintenance', 'disableDailyMaintenance_'))
    .addSubMenu(ui.createMenu('Outreach Automation').addItem('Prepare Selected Prospect', 'menuPrepareSelectedProspect_').addItem('Prepare Eligible Prospects', 'menuPrepareEligibleProspects_').addItem('Resume Batch', 'menuResumeOutreachBatch_').addItem('Outreach Automation Status', 'menuOutreachAutomationStatus_').addItem('Configure API Status', 'menuConfigureApiStatus_'))
    .addSeparator()
...
function onEdit(e) {
  ...
  } catch (err) { Logger.log('onEdit failed: ' + err.message); }
}
...
function buildRCSCRM() {
  ...
  SHEET_DEFS.forEach(function(def) {
    ...
    if (def.name === 'Dashboard') { buildDashboard_(sheet); return; }
...
function maybeRepurposeDefaultSheet_(ss) {
  ...
  const only = sheets[0];
  if (/^Sheet ?1$/i.test(only.getName()) && only.getLastRow() === 0 && only.getLastColumn() === 0 && !ss.getSheetByName('Dashboard')) only.setName('Dashboard');
}
function ensureHeaders_(sheet, headers) {
  ...
  if (!existingHeaders.length) { sheet.getRange(1,1,1,headers.length).setValues([headers]); return; }
  const existingSet = {};
...
function formatDataSheet_(sheet, numCols) {
  if (!numCols) return;
  freezeHeaderRow_(sheet); styleHeaderRow_(sheet, numCols); applyAlternatingBanding_(sheet, numCols); applyBasicFilter_(sheet, numCols); autoResizeColumns_(sheet, numCols);
}
function freezeHeaderRow_(sheet) { sheet.setFrozenRows(1); }
function styleHeaderRow_(sheet, numCols) { sheet.getRange(1,1,1,numCols).setBackground(HEADER_BG).setFontColor(HEADER_FG).setFontWeight('bold').setVerticalAlignment('middle'); }
...
function applyBasicFilter_(sheet, numCols) { const existing = sheet.getFilter(); if (existing) existing.remove(); sheet.getRange(1,1,Math.max(sheet.getLastRow(),1),numCols).createFilter(); }
function autoResizeColumns_(sheet, numCols) { sheet.autoResizeColumns(1, numCols); }

// ---------------------------------------------------------------------------
// Safety helpers. Other CRM modules should call these for production-only
// actions/reporting as they are extended. Existing Health/Command Center
// exclusion logic remains authoritative for current reporting paths.
// ---------------------------------------------------------------------------
function isInternalTestBusiness_(business) { return String(business || '').trim().toLowerCase() === RCS_CRM_SAFETY.INTERNAL_TEST_BUSINESS.toLowerCase(); }
function isInternalTestWebsite_(website) { return String(website || '').trim().toLowerCase().indexOf(RCS_CRM_SAFETY.INTERNAL_TEST_DOMAIN.toLowerCase()) !== -1; }
function isInternalTestRecord_(business, website) { return isInternalTestBusiness_(business) || isInternalTestWebsite_(website); }
function shouldExcludeFromProduction_(business, website) { return isInternalTestRecord_(business, website); }
function assertNotProductionRecord_(business, website) {
  if (shouldExcludeFromProduction_(business, website)) throw new Error('Roman Creative Studio is an internal CRM test account and cannot be used in production-only operations.');
  return true;
}
function crmWebsiteMutationAllowed_() { return false; }
function assertCrmOnlyOperation_() {
  if (RCS_CRM_SAFETY.WEBSITE_MUTATION_ALLOWED !== false || crmWebsiteMutationAllowed_() !== false) throw new Error('CRM safety violation: website mutation/deployment is disabled.');
  return true;
}
function ensureInternalTestBusinessSetting_() {
  const settings = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Settings');
  if (!settings) return;
  settings.getRange(1,14,2,3).setValues([
  ...
  settings.getRange(1,14,1,3).setBackground(HEADER_BG).setFontColor(HEADER_FG).setFontWeight('bold');
}
...
function runRcsSafetyCheck_() {
  ...
  if (RCS_CRM_SAFETY.WEBSITE_MUTATION_ALLOWED !== false || crmWebsiteMutationAllowed_() !== false) failures.push('Website mutation policy is not locked off.');
...
function notify_(message) { try { SpreadsheetApp.getUi().alert(message); } catch(e) { Logger.log(message); } }
