...
function buildRCSCRM() {
  ...
  // New RCS internal-test configuration is additive and does not overwrite
  // existing CRM records or schema columns.
...
// ---------------------------------------------------------------------------
// RCS INTERNAL TEST + SAFETY LAYER — ADDITIVE TO THE ORIGINAL CODE ABOVE
// ---------------------------------------------------------------------------
...
function isInternalTestWebsite_(website) {
  return String(website || '').trim().toLowerCase()
    .indexOf(RCS_CRM_SAFETY.INTERNAL_TEST_DOMAIN.toLowerCase()) !== -1;
}
...
function assertNotProductionRecord_(business, website) {
  if (shouldExcludeFromProduction_(business, website)) {
    throw new Error(
      'Roman Creative Studio is an internal CRM test account and cannot be used in production-only operations.'
    );
  }
  return true;
}

// Explicit hard boundary: the CRM may audit/read a website through the
// existing Website Audit module, but this CRM has no authority to edit,
// deploy, publish, delete, or otherwise mutate website source/hosting.
...
function assertCrmOnlyOperation_() {
  if (RCS_CRM_SAFETY.WEBSITE_MUTATION_ALLOWED !== false ||
      crmWebsiteMutationAllowed_() !== false) {
    throw new Error(
      'CRM safety violation: website mutation/deployment is disabled.'
    );
  }
  return true;
}
...
function ensureInternalTestBusinessSetting_() {
  ...
  // N:P is intentionally outside the primary Settings dropdown-list area
  // and existing GitHub Sync / Automation status panels.
...
function runRcsSafetyCheck_() {
  ...
  if (RCS_CRM_SAFETY.CANONICAL_CRM_FOLDER !== 'crm') {
    failures.push('Canonical CRM folder policy is incorrect.');
  }
  if (RCS_CRM_SAFETY.WEBSITE_MUTATION_ALLOWED !== false ||
      crmWebsiteMutationAllowed_() !== false) {
    failures.push('Website mutation policy is not locked off.');
  }
  if (!isInternalTestBusiness_('Roman Creative Studio')) {
    failures.push('Internal test business matcher failed.');
  }
  if (!isInternalTestWebsite_('https://romancreativestudio.co')) {
